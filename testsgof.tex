...
\begin{figure}[!ht]\includegraphics[trim={0cm 0cm 0 0},clip, keepaspectratio,width=0.99\textwidth]{mostpower}\label{fig:mostpower}
\end{figure}
\end{column}
...
\end{itemize}
	\end{column}
	\begin{column}{0.5\textwidth}
\begin{figure}[!ht]\begin{subfigure}[b]{0.99\textwidth}\centering\includegraphics[trim={0cm 0 0 0},clip, width=0.99\textwidth]{powerconsistent}\label{fig:powerconsistent}\end{subfigure}~\begin{subfigure}[b]{0.99\textwidth}\centering\includegraphics[trim={0cm 0 0 0},clip,width=0.99\textwidth]{biasedtest}\label{fig:biasedtest}\end{subfigure}\end{figure} 
...
Compare observed data $\vec{n}=(n_1,\ldots,n_k)$ with predicted data $\vec{\nu}=(\nu_1,\ldots,\nu_k)$: $\chi^2=\sum_i^k\frac{(n_i-\nu_i)^2}{\sigma_i^2}$, $\sigma_i^2=\var{n_i}$.
\begin{itemize}
\item $\sum_in_i=N$ fixed: $\sum_jp_j=1$, $\prob{(Y_1=y_1,\ldots,Y_{k}=y_{k})}=\frac{n!}{y_1!*\ldots*y_{k}!}p_1^{y_1}\ldots p_{k}^{y_{k}}$, parameter space $\Omega=\{(p_1,\ldots,p_{k-1}\})\in \real{k}$. $H_0: p_i=\pi_i$ vs $K: p_i\neq\pi_i$:
statistica $Q_n=\sum_j^{k}\frac{(Y_j-n\pi_j)^2}{n\pi_j}\to\frac{1}{N}\sum^{k}\frac{n_i^2}{p_i}-N$ ha asintotic. pdf $\chi^2(k-1)$, quindi se $c_{k-1,1-\alpha}$ $1-\alpha$-quantile di $\chi^2(k-1)$ il test che rifiuta H quando $Q_n>c_{k-1,1-\alpha}$ ha asintoticamente valore critico $\alpha$.
