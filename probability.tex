...
\subsection{Significato di probabilit\'a??}\linkdest{aximpl}
...
RV: \sout{Deterministic} Stochastic phenomena (\mykeyword{frequentista} Possibili risultati di una misura/\mykeyword{soggettivista}: elementi del sample space sono ipotesi/proposizioni)
\end{block}
...
\begin{wordonframe}{Esempio importanza ensamble (selection bias)}
	...
	Suppongo che l'intervallo di tempo tra mio arrivo alla fermata e passaggio autobus precedente sia distribuito uniformemente in $[0,T]$ e che la probabilit\'a di tempo $\Delta T$ fra 2 autobus sia \mykeyword{pdf esponenziale} $p(t;\lambda)=\lambda\exp{-\lambda t}$: $\E{(t_a)}=\E{(tx)}=\E{(t)}\E{(x)}=\frac{T}{2}$.
	\begin{columns}[T]
		\begin{column}{0.4\textwidth}
		...
		\end{column}
	\end{columns}
	\mykeyword{Tempo medio di attesa in quale ensamble?}
	\begin{columns}[T]
		\begin{column}{0.55\textwidth}
			\begin{align*}
			&\frac{\sum_i\sum_j\frac{T_{ij}}{n_i}}{N}\neq\frac{\sum_{ij}T_{ij}}{\sum_in_i}
			\end{align*}
			\mykeyword{Ex.: correlazione $x-T_i-n_i$?}
...
\subsection{Variabili casuali: osservabili}\linkdest{RV}
...
\item Expectation value of product of RV:%https://en.wikipedia.org/wiki/Product_distribution:Expectation of product of random variables
\begin{align*}
...
\begin{wordonframe}{Rapporto di due RV $U([0,1])$}
	\begin{columns}[t]
		...
		\begin{column}{0.4\textwidth}
			...
			\end{align*}
			\mykeyword{EX: Rapporto RV uniformi}
		\end{column}
	\end{columns}
\end{wordonframe}
...
\begin{wordonframe}{Rapporto di gaussiane}
	...
	&\E{(z^{2n+1})}=Ind
	\end{align*}
	\begin{block}{\mykeyword{Somma di Cauchy}: $S=x+y$}
...
\begin{block}{pdf di \mykeyword{Cauchy}: non vale LLN}
\mykeyword{Parametri larghezza e posizione}: $\prob{(x)}=\frac{1}{\pi\Gamma}\frac{1}{1+(\frac{x-m}{\Gamma})^2}$.
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\begin{align*}
...
\mykeyword{Chebyshev ineq}: $\prob{(|X-\mu|\geq k\sigma)}\leq\frac{1}{k^2}$.
\mykeyword{Probability convergence}: Una sequenza di RV $\{X_n\}\to X$ in probabilit\'a se $\lim_{n\to\infty}\prob{(|X_n-X|)>\epsilon}$=0.
    \end{column}
    \end{columns}
\end{frame}
...
\end{align*}
\mykeyword{Ex: media e varianza Poissoniana}
\end{frame}
...
\end{align*}
La \mykeyword{pdf del tempo fra due eventi di una pdf poissoniana \'e esponenziale}: 
Hyperexponential:
Erlang
\end{frame}
...
\begin{columns}[T]
	...
	\end{column}
	\begin{column}{0.5\textwidth}
	\begin{block}{\mykeyword{Distribuzione massimo n misure estratte da $U(0,m)$}: pdf di $x_M$}
