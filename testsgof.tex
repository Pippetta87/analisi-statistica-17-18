...
\begin{block}{Partizione di X in regione di accettanza/critica}
Decisione basata su valore di RV X la cui pdf $P_{\theta}$ che appartiene a una classe di pdf $\mathcal{P}=\{P_{\theta}, \theta\in\Omega\}$. Se il parametro \'e noto sappiamo con certezza se vera/falsa: $\mathcal{P}=H\cup K$ denotano classi mutualmente esclusive di pdf corrispondenti a $\Omega=\Omega_H\cup\Omega_K$. Se indico $d_0/d_1$ la decisione di accettare/rifiutare assegno ogni $x\in X$ a regioni complementari $S_0/S_1$ regione di accettanza/critica.
\end{block}
\end{frame}
...
(falso positivo), scarto $H_0$ quando \'e vera (loss,size,taglia,livello di significativit\'a $\alpha=\prob{(x\in C|H_0)}=\prob_0{(x\in C)}$.
\end{block}
\end{column}
\begin{column}{0.55\textwidth}
\begin{block}{\keyword{Prob. Errore tipo II $\beta$}}
(falso negativo, contaminazione): probabilit\'a di accettare erroneamente $H_0$, $\prob{(x\in\overline{C}|\not{H_0})}=\beta$. Potenza del test $1-\beta=\prob{(x\in C|H_1)}$ \'e la capacit\'a di distinguere ipotesi diverse da $H_0$ 
\end{block}
\end{column}
\end{columns}
...
\begin{itemize}
	\item \keyword{Consistenza}: $\lim_{N\to\infty}\prob{(\vec{X}\in w_{\alpha}|H_1)}=1$ non \'e detto che sia uniforme.
...
Real parameter family of density $p_{\theta}(x)$ is said to have monotone LR if exists statistics $T(x)$ st for $\theta<\theta'$ le distribuzioni sono distinte e $\frac{P_{\theta'}(x)}{P_{\theta}(x)}$ \'e funzione non decrescente di $T(x)$
\end{block}
\end{frame}
...
\label{fig:oneone2sided}\end{figure}
...
Ipotesi $H_0$ definisco $\alpha=\prob{(T(x)=\non{(H_0)}|H_0)}=\prob{(T(x)\in C|H_0)}$, non specifico ipotesi alternative.
\begin{itemize}
\item Test statistic: function of data which measure ''distance'' between data and hypothesys
\item Map statistic to P-value: suppose $\frac{p_1(X)}{p_0(X)}$ continuo, $\alpha<\alpha': S_{\alpha}\subset S_{\alpha'}$, \keyword{p-value} p is smallest significance level at which hypothesis would be rejected for the given observation.
\end{itemize}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\[P_X=\sum_{x:t>t_0=t(X_0)}\prob{(x|H_0)}\to\inf\{\alpha: X\in C_{\alpha}\}\]
\end{column}
\begin{column}{0.5\textwidth}
