...
\begin{align*}
&\xi(\theta)=\invers{N}\sum_ia(X_i,\theta)\xrightarrow{N\to\infty}\E{[a(X,\underbrace{\theta}_{impl})]}=\int a(X,\theta)f(X;\theta)\,dX=h(\theta)
\end{align*}
$a(X)$ ha varianza finita: per LLN sample mean is C-estimator of $a(X)$. We find $a(X,\theta)$ in maniera che $h(\theta_0)=0$ per il valore vero di $\theta_0$, $\xi(\hat{\theta})=0$, stimatore implicito:
\begin{align*}
...
In practice one find $a(X_i,\theta)$ extremizing other function g: $a(X_i,\theta)=\PDof{\theta}g(X_i,\theta)$: $\hat{\theta}=\invers{N}\sum_i^Ng(X_i,\theta)$ is C-estimator of $\theta_0$ and is a root of $\xi(\theta)=0$ and we construct $a(X,\theta)$
\begin{align*}
&a(X_i,\theta_0)=\PDy{[g(X_i,\theta)-\E{[g(X_i,\theta)]}]}|_{\theta_0}\\
&\E{(a(X_i,\theta_0))}=0
\end{align*}
\end{frame}
