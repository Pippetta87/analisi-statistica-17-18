...
\begin{frame}{Coverage}
    Probabilit\'a che intervallo contenga vero valore del parametro.
\end{frame}
...
\subsection{Rivelatore difettoso? (09/09/21)}
\begin{frame}{Poissoniana 10 misure indipendenti: pvalue}
    njkj
\end{frame}
...
\begin{frame}{Stima ML di $\beta$}
    \begin{itemize}
        \item Asintoticamente MLE \'e efficiente: $\var{(\beta)}=\invers{(I(\vec{x}))}$
            \begin{align*}
                &I=\E{[(\PDy{\beta}{\ln{L}}|_{\beta_0})^2]}=-\E{[\PtwoDy{\beta}{\ln{L}}|_{\beta_0}]}=\sum_i\E{[k_i]}t_i=\frac{(-t_i^2)}{(\alpha t_i+\beta t_i^2)}=\sum_i\frac{t_i^2}{\alpha}
            \end{align*}
                L'equazione di likelihood determina lo stimatore (massimo assoluto di $L$):
                \[\PDof{\beta}\ln{L}=-\sum_it_i^2+\sum_i\frac{k_it_i^2}{\alpha t_i+\beta t_i^2}\]
    \end{itemize}
\end{frame}

\begin{frame}{Stima di $(\alpha,\beta)$ (metodo momenti?) e GOF}
    \begin{itemize}
            \item Stimatori metodo momenti (Equazione di likelihood ha calcoli complessi)
                \begin{align*}
                    &a
                \end{align*}
    \end{itemize}
...
\item Limiti frequentisti al $\cl=90\%$ sul numero di facce - \keyword{Scelta algoritmo ordinamento per costruzione banda di Neyman}: prob. ottenere n dal lancio di qualunque dado $\prob{(n|D_i)}=\frac{1}{i},\ n\leq i$ - \keyword{limite superiori - ord. decrescente} in osservabile n ($n=1\Rightarrow i\leq10$ al $\cl=90\%$)
\begin{figure}
	\centering
...
\begin{frame}{Banda di Neyman con LR ordering a l\'a FC}
    \keyword{Likelihood ratio ordering} $O(D_i,n)=\lr(n)=\frac{\prob{(n;D_i)}}{\sup_{D_i'}\prob{(n;D_i')}}$: tabella 2 mostra a sinistra banda di confidenza e a destra i valori di LR.
\begin{figure}
	\centering
...
\begin{frame}{Limiti di confidenza su numero eventi annuali: p-ordering, limite inferiore, LR-ratio di $\lambda$}
    Stima intervallare di $\lambda=\frac{1}{\tau}$ e $\prob{(t;\lambda)}=\lambda\exp{-\lambda t}$: pdf \'e monotona decrescente in t quindi \keyword{CI basato su p-ordering} equivale a $o(t)=-t$, $t\leq t_{max}(\lambda)$
\begin{align*} 
...
Ma limite inferiore a un fenomeno nuovo! \keyword{CI per limite inferior e $o(t)=t$}:
\begin{equation*}
...
\keyword{Prior impropria}: in mancanza di una scala definita utilizzo prior uniforme in $[0,\infty]$ (non normalizabile). Devo normalizare la posterior:
\begin{align*}
...
\begin{columns}[T]
	\begin{column}{0.5\textwidth}
        \keyword{CI - Limite bayesiano inferiore}
\begin{equation*} 
\int_{\lambda_{min}}^{\infty}t_m^2\lambda\exp{-t_m\lambda}\,d\lambda=\cl{}
\end{equation*}
	\end{column}
	\begin{column}{0.5\textwidth}
        \keyword{CI - Limite bayesiano bilaterale simmetrico}:
\begin{align*}
...
\end{column}
\end{columns}
\keyword{CI - Limite bayesiano con posterior ordering}:
\begin{align*}
...
\begin{frame}{Stimatori usando momenti da sequenza temporale discreta}
...
\begin{align*}
&\E{[\mu_0]}=\sum_i^5n_i\quad\E{[\mu_1]}=\sum_i^5(i-1)n_i
\end{align*}
\end{frame}

\section{Math tools}\linkdest{mathtools}
...
&\binom{n}{m}\binom{m}{k}=\binom{n}{k}\binom{n-k}{m-k}\tag{subsubSet}
\end{align*}
\end{column}
\begin{column}{0.6\textwidth}
\begin{align*}
...
&\sum_{k=0}^n\binom{n}{k}=2^n\tag{row-sum}\\
&\sum_{r=0}^n\binom{r}{c}=\binom{n+1}{c+1}\tag{column-sum}\\
&\sum_{k=0}^n\binom{r+k}{k}=\binom{r+n+1}{n}\tag{SE-Diagonal}\\
&\sum_{k=0}^n\binom{n-k}{m-k}=\binom{n+1}{m}\tag{NW-Diagonal}\\
&\sum_{k=0}^n\binom{n-k}{k}=f_{n+1}\tag{Fib-NE-Diag}
\end{align*}
\end{column}
	\end{columns}
\end{wordonframe}
...
\begin{frame}{Identities}
	...
	\begin{column}{0.5\textwidth}
        \begin{itemize}
            \item \keyword{Stirling's approximation}: $n!=\sqrt{2\pi n}(\frac{n}{e})^n$
        \end{itemize}
	\end{column}
	\begin{column}{0.5\textwidth}
	\keyword{Variance symmetric formula}
	\begin{align*} 
    ...
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            \begin{align*}
                ...
                &\sum_{k=1}^Nr^{k-1}=r^0+\ldots+r^{N-1}\\
                &(1-r)\sum_{k=1}^Nr^{k-1}=(1-r)(1+\ldots+r^{N-1})\\
                &=1+\ldots+r^{N-1}-r^1-\ldots-r^N\\
                &1-r^N\\
                &\sum_{k=0}^{N}r^{2k}=\frac{1-r^{2N+2}}{1-r^2},\ \sum_{k=0}^{N}r^{2k+1}=R\frac{1-r^{2N+2}}{1-r^2}
            \end{align*}
        \end{column}
        \begin{column}{0.5\textwidth}
...
\begin{frame}{Bessel functions}
    \begin{align*}
        &z^2\TtwoDy{z}{w}+z\TDy{z}{w}+(z^2-\nu^2)w=0\tag{Bessel eq. order $\nu$}\\
        &w(z)=\sum_{r=0}^{\infty}a_rz^{\alpha+r}=AJ_{\nu}(z)+BJ_{-\nu}(z)=AJ_{\nu}(z)+BY_{\nu}(z)=AH_{\nu}^{(1)}+BH_{\nu}^{(2)}\\
        &J_{\nu}(z)=\sum_{r=0}^{\infty}\frac{(-)^r(z/2)^{\nu+2r}}{r!\Gamma(\nu+r+1)}\tag{$2\nu\notin-\mathds{N}$}\\
        &J_{-\nu}(z)=\sum_{r=0}^{\infty}\frac{(-)^r(z/2)^{-\nu+2r}}{r!\Gamma(-\nu+r+1)}\tag{$2\nu\notin\mathds{N}$}\\
        &Y_{\nu}(z)=\lim_{\alpha\to\nu}\frac{\cos{(\alpha\pi)}J_{\alpha}(z)-J_{-\alpha}(z)}{\sin{(\alpha\pi)}}\tag{Bessel II kind order $\nu$}\\
        &H_{\nu}^{(1/2)}(z)=J_{\nu}(z)\pm iY_{\nu}(z)\tag{Hankel(Bessel of III kind)}\\
    \end{align*}
\end{frame}

\begin{frame}{Modified Bessel functions}
    \begin{align*}
        &z^2\TtwoDy{z}{w}+z\TDy{z}{w}-(z^2+\nu^2)w=0\tag{$z\to iz$: modified Bessel eq. order $\nu$}\\
        &I_{\nu}(z)=\sum_{r=0}^{\infty}\frac{(z/2)^{\nu+2r}}{r!\Gamma(\nu+r+1)}\propto J_{\nu}(iz)\\
        &K_{\nu}(z)=\frac{\pi}{2}\lim_{\alpha\to\nu}\frac{I_{-\alpha}(z)-I_{\alpha}(z)}{\sin{(\alpha\pi)}}\tag{modified Hankel(mod Bessel of III kind)}\\
        &w(z)=AI_{\nu}(z)+BK_{\nu}(z)
    \end{align*}
\end{frame}
\end{document}
