...
\begin{itemize}
\item \keyword{Distribuzione di probabilit\'a cumulante}: $F(x)=P(x_n<x)=\int_{-\infty}^xf(x')\,dx'$: $f(x)=\TDy{x}{F(x)}$.
\item $\alpha$-point, \keyword{quantile of order $\alpha$}: $F(x_{\alpha})=\alpha$.
\item Probabilit\'a $([x,x+dx],y)=(A)$: \keyword{marginal pdf} for x $\prob{(A)}=\int f(x,y)\,dxdy=f_x(x)\,dx$, $(x,[y,y+dy])=(B)$.
$\prob{(A\cap B)}=f(x,y)\,dx\,dy$; \keyword{Probabilit\'a condizionata} $\prob{(B|A)}=\frac{\prob{A\cap B}}{\prob{A}}=\frac{f(x,y)\,dx\,dy}{f_x(x)\,dx}$; conditional pdf for y given x $h(y|x)=\frac{f(x,y)}{f_x(x)}$, simile per $g(x|y)=\frac{f(x,y)}{f_y(y)}$: \keyword{T Bayes} $g(x|y)=\frac{h(y|x)f_x(x)}{f_y(y)}$ and \keyword{law of total prob.}: $f_x(x)=\intsinf{}g(x|y)f_y(y)\,dy$.
...
\end{align*}
\keyword{Ex: media e varianza Poissoniana}
\end{frame}
