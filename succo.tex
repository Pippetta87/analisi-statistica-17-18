...
\end{align*}
\hl{$a$ ha varianza finita}, \hl{$h(\theta_0)=0$}: per LLN uno zero di $\xi(\theta)$ converge a $\theta_0$, $\xi(\hat{\theta})=0$ \keyword{stimatore implicito}.
\begin{columns}[T]
\begin{column}{0.6\textwidth}
...
\begin{figure}
	\centering
	\includegraphics[width=0.99\textwidth,keepaspectratio]{estimpl}
	\label{fig:RVfunc}
\end{figure}
\end{column}
\end{columns}
