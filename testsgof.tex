...
Sia $H_0: \vec{\theta}\in\nu$, $H_0: \vec{\theta}\in\overline{\nu}$; test statistic maximum likelihood ratio: $\lambda=\frac{\max_{\vec{\theta}\in\nu}L(\vec{X},\theta)}{\max_{\vec{\theta}\in\overline{\nu}}L(\vec{X},\theta)}$, critical region and significance level: $\alpha=\sup{\prob{(\lambda(\vec{X})\leq k;\theta\in\theta_0)}}$.
...
Per $\Omega_0\subset\Omega_1\subset\real^p$: $\forall\theta\in\Omega_0$ $-2\log{\lambda}\to\chi^2_{q-r}$
\end{block}
\end{frame}
