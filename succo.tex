...
\begin{block}{\mykeyword{pdf di $X_{(2)}$ per N estrazioni da pdf uniforme}}
\begin{align*}
...
\mykeyword{pdf di $x_M$} \'e $\frac{N}{m}(\frac{x_M}{m})^{N-1}$: stima di \mykeyword{massima likelihood di m da statistica $x_M$}: $\hat{m}=x_M$; stimatore unbiased $\hat{m}'=\frac{N+1}{N}\hat{m}$.
...
L'\mykeyword{efficienza relativa di due stimatori} si ottiene:
\begin{equation*}
\epsilon_r=\frac{\var{[\hat{m'}_{M}]}}{\var{[\hat{m'}_{x_2}]}}
\end{equation*}
\end{block}
\begin{block}{\mykeyword{Combinazione likelihood 2 misure di $x_2$}}
...
\end{block}
\begin{block}{\mykeyword{Test bilaterale di ipotesi composte: dati da pdf uniforme con $m_1\neq m_2$?}}
Likelihood ratio test:
\begin{equation*}
\lambda(x_1',x_2'')=-2\log{\frac{\sup_m{p(x_2',x_2'';m)}}{\sup_{m_1,m_2}{p(x_2',x_2'';m_1,m_2)}}}
\end{equation*}
\mykeyword{chiarire cosa significa massimizzare likleihood combinata per pi\'u parametri o uno!}
\begin{align*}
...
\end{align*}
\mykeyword{Likelihood ratio monotona (asintoticamente): statistica $t(x_2',x_2'')=x_2''/x_2'$}: il test LR \'e della forma $x_2''/x_2'>q_{\alpha}$
\mykeyword{Monotone LR in statistic $T(x)$} and \mykeyword{Karlin-Rubin theorem}
\end{block}
\end{wordonframe}
...
Il \mykeyword{MLE trasforma in modo banale sotto cambio di variabile}: $\hat{\lambda}=\hat{\frac{1}{\tau}}=\SI{9.3}{\counts\per\year}$; usando formula per propagazione errori si ha
\begin{equation*}
\sigma(\lambda)=\sqrt{\var{[\hat{\lambda}]}}\approx\frac{1}{\tau^2}\sqrt{\var{[\tau]}}
\end{equation*}
Tuttavia l'\mykeyword{approssimazione quasi lineare del cambio di variabile} \'e una pessima approssimazione sull'ampio intervallo di incertezze della variabile originale.
...
\mykeyword{bande di confidenza per pdf esponenziale}
Stima intervallare di $\lambda=\frac{1}{\tau}$ e $\prob{(t;\lambda)}=\lambda\exp{-\lambda t}$. \mykeyword{probability ordering}: pdf \'e monotona decrescente in t quindi p-ordering equivale a $o(t)=-t$, $t\leq t_{max}(\lambda)$
\begin{align*}
&\int_0^{t_{max}}\lambda\exp{-\lambda t}\,dt=\cl{}\\
&t_{max}(\lambda)=-\frac{\log{(1-\cl{})}}{\lambda}
\end{align*}
$t_{max}(\lambda)$ decrescente in $\lambda$, da $t<t_{max}(\lambda)$ si vede che \mykeyword{la banda di confidenza \'e della forma $t<t_{max}(\lambda)$ porta a limite $\lambda\leq\lambda_{max}$} (limite superiore); l'inversione porta a $\lambda_{max}=-\frac{\log{(1-\cl{})}}{t_m}$.
...
\mykeyword{prior impropria}: in mancanza di una scala definita utilizzo prior uniforme in $[0,\infty]$ (non normalizabile). Devo normalizare la \mykeyword{posterior}:
\begin{align*}
&\pi(\lambda|t_m)\propto \lambda\exp{-t_m\lambda}=L_{t_m}(\lambda)\\
&\pi(\lambda|t_m)=t_m^2\lambda\exp{-t_m\lambda}
\end{align*}
quindi \mykeyword{limite bayesiano inferiore}
\begin{equation*}
\int_{\lambda_{min}}^{\infty}t_m^2\lambda\exp{-t_m\lambda}\,d\lambda=\cl{}
\end{equation*}

\mykeyword{limite bayesiano bilaterale simmetrico}:
\begin{align*}
...
\mykeyword{limite bayesiano utilizzando posterior ordering}:
\begin{align*}
