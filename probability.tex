...
\begin{frame}{Significato di evento. Eventi congiunti. Distribuzione cumulante}
\begin{block}{\keyword{Distribuzione di probabilit\'a cumulante}}
$F(x)=P(x_n<x)=\int_{-\infty}^xf(x')\,dx'$: $f(x)=\TDy{x}{F(x)}$; $\alpha$-point, \keyword{quantile of order $\alpha$}: $F(x_{\alpha})=\alpha$
\end{block}
\begin{block}{\keyword{Eventi congiunti e probabilit\'a condizionata}}
Probabilit\'a $([x,x+dx],y)=(A)$: \keyword{marginal pdf} for x $\prob{(A)}=\int f(x,y)\,dxdy=f_x(x)\,dx$, $(x,[y,y+dy])=(B)$.
