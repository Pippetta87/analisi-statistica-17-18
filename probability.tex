...
\begin{wordonframe}{Esempio importanza ensamble (spazio campionario)}
Suppongo che l'intervallo di tempo tra mio arrivo alla fermata e passaggio autobus precedente sia distribuito uniformemente in $[0,T]$ e che la probabilit\'a di tempo $\Delta T$ fra 2 autobus sia $p(t;\lambda)=\lambda\exp{-\lambda t}$
\begin{block}{Tempo medio di attesa in quale ensamble?}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
Probabilit\'a arriva prima di t:
\begin{align*}
&F(t;\lambda)=\int_0^tp(t';\lambda)d\,t\\
&=1-F(t)=1-\exp{-\lambda t}
\end{align*}
\end{column}
\begin{column}{0.5\textwidth}
...
\begin{frame}{Generazione pdf da distribuzione uniforme}
