...
&P(\chi^2_2)=\frac{\exp{-\frac{x^2}{2}}}{2}\tag{$\chi^2$ with dof 2 is exponential with $\lambda=\frac{1}{2}$}\\
	&(p_1,\ldots,p_N)\to Q=\sum^N-2\log{p_i}\tag{$*2$ we have sum of N $\chi_2^2$}\\%Erlang $\lambda=1/2$
	&\approx\prob{(\chi^2;\nu=2*N)}=\frac{1}{2\expy{2N}\Gamma(2N)}x\expy{2N-1}\exp{-x/2}\tag{sum N $\chi^2_2$ is $\chi^2_{2N}$}\\
...
\end{align*}
$c_{N-1,1-\alpha}$ is $1-\alpha$-quantile di $\chi^2(k-1)$: test rifiuta $H_0$ quando $Q_N>c_{N-1,1-\alpha}$ ha asintoticamente valore critico $\alpha$.
\end{frame}
