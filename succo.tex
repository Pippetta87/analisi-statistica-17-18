...
\begin{frame}{Convergenza di RV}
\begin{block}{Chebychev ineq}
\keyword{Chebyshev ineq}: $\prob{(|X-\mu|\geq k\sigma)}\leq\frac{1}{k^2}$.
\end{block}
\begin{block}{\keyword{Probability convergence}}
Una sequenza di RV $\{X_n\}\to X$ in probabilit\'a se $\lim_{n\to\infty}\prob{(|X_n-X|)>\epsilon}$=0.
\end{block}
\end{frame}

\begin{frame}{Calcolo pdf $x_{max}$, $x_{min}$ per $U([0,m])$}
...
\begin{align*}
&\var{\overline{X}_n}=\var{\frac{1}{n}(X_1+\ldots+X_n)}=\frac{1}{n^2}\var{(X_1+\ldots+X_n)}=\frac{n\sigma^2}{n^2}=\frac{\sigma^2}{n}\\
&\prob{(|\overline{X}_n-\mu|>\epsilon)}\leq\frac{\sigma^2}{n\epsilon^2}
\end{align*}

\end{frame}

\begin{frame}{Levy theorem: $\overline{X}_n\to\mu$}
