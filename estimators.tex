...
\begin{wordonframe}{Gaussiana: media ignota, varianza nota}
\begin{align*}
...
\begin{wordonframe}{Gaussiana: media nota, varianza ignota}
...
\begin{wordonframe}{\keyword{Media aritmetica \'e statistica sufficiente per parametro binomiale, poissoniana, esponenziale}}
\begin{align*}
...
dove $a(X)$ \'e qualsiasi funzione di X con varianza finita: per LLN sample mean is C-estimator of $a(X)$.
...
\begin{frame}{\keyword{Stimatori impliciti} come generalizzazione del metodo dei momenti}
 We find $a(X,\theta)$ in maniera che $h(\theta_0)=0$ per il valore vero di $\theta_0$, $\xi(\hat{\theta})=0$, stimatore implicito:
\begin{align*}
...
\end{align*}
The quantity $\PDof{\theta}\ln{L(\vec{X},\theta)}\to G(0,NI(\theta))$.
\end{frame}
...
\begin{frame}{MLE in pratica: binned data and extended likelihood}\frameintoc
\begin{block}{binning}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
N fissato
...
\end{column}
\begin{column}{0.5\textwidth}
\begin{figure}[!ht]\includegraphics[trim={0cm 0cm 0 0},clip, keepaspectratio,height=0.49\textheight]{flipflopping}\label{fig:pdfestexp}\end{figure}
\end{column}
\end{columns}
