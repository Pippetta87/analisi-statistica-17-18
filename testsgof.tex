...
\begin{itemize}
\item $\sum_in_i=N$ fixed: $\sum_jp_j=1$, $\prob{(Y_1=y_1,\ldots,Y_{k+1}=y_{k+1})}=\frac{n!}{y_1!*\ldots*y_{k+1}!}p_1^{y_1}\ldots p_{k+1}^{y_{k+1}}$, parameter space $\Omega=\{(p_1,\ldots,p_k\})\in \real{k}$. $H_0: p_i=\pi_i$ vs $K: p_i\neq\pi_i$:
