...
Per distribuzione a supporto non compatto esiste S tale che $\dim{(S)}<\dim{(X)}$ iff: pdf appartiene alla \keyword{famiglia esponenziale} e supporto non dipende dal parametro.
\begin{equation*}
...
\begin{frame}{\keyword{Credibility (della posterior)}}
	\begin{align*}
        &\cred{(x)}=\int_{f(\vec{x};\vec{\theta})>c(\vec{\theta})}\Pi(\mu|x)\,d\mu=\int_{\ldots}\frac{\prob{(x|\mu)}\prob{(\mu)}}{\int\,d\mu}\,d\mu>\cl{}
	\end{align*}
...
\end{frame}
    ...
    \begin{frame}{\keyword{Relation between Pois cumulant and $\chi^2$}}
        \begin{align*}
            ...
            %&P(n\geq r)=1-\prob{(n\leq r-1)}=1-\frac{\gamma(a=\nu,x)}{\Gamma(a=\nu)}=\frac{\Gamma(r)-\gamma(r,\nu)}{\Gamma(r)}\\
            %&=\frac{1}{\Gamma(r)}(\intzi{}t^{r-1}\exp{-t}\,dt-\int_{\lambda}^{\infty}t^{w-1}\exp{-t}\,dt)=\frac{1}{\Gamma(r)}\int_0^{\nu}t^{r-1}\exp{-t}\,dt=\prob{(Y\leq\nu)},\ Y\sim\Gamma(r,1)
