...
\begin{frame}{Bayesian Adaptive psychometric function}
    \begin{itemize}
        ...
        \item Termination r ule: when CI for location of threshold is smaller than given size - $L_n(T)=Q_n(T)-Q_0(T)$, to test $T=T_j$ we can use LR-test $L(\hat{T}_{MLE})-L(T_j)\geq0.5\chi^2_{\alpha,dof=1}$, outside the interval $T=T_j$ is rejected - T equals peak of $L(T)$ folowing last trials.
    \end{itemize}
\end{frame}
